...
int main(int argc, char *argv[]) {
	...
	s_sketch.deltaTime = 0;
	s_sketch.font = &defaultFont;

	lithUpdateTime();

	__setContext(&s_sketch, &s_app);
	setup();
	...
	while (running) {
		...
		s_sketch.deltaTime = lithDeltaTime();
		s_sketch.totalTime = lithTotalTime();
